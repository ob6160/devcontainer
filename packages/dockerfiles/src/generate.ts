...
export async function writeResult(imagenameStack: string[], parentReadMes: string[], parentDockers: string[]) {

    const stack = imagenameStack.join('-');

    if (processed[stack] === true) return true;  // do  not process the same stack twice
    ...
    if (
        !imagenameStack.includes('dotnet') && 
        ...
        await fs.writeFile(`${newPath}/push.sh`, `yarn generate && cd ../zsh && docker build . -t zolika84/devcontainer:${version}-${imageName} && docker push zolika84/devcontainer:${version}-${imageName}`);
    ...
    FROM zolika84/${fromImageName}:${version}\r\n\r\n` : '';
        ...
        await fs.writeFile(`${newPath}/Dockerfile`, `### zolika84/devcontainer:${version}-${imageName}
######
`+
            parentDockers.join('\n\n'));

        const newReadme = `# zolika84/devcontainer:${version}-${imageName}\r\n`;
        await fs.writeFile(`${newPath}/README.md`, `## ${newReadme}${readme}`);
...
function cutout(arr: string[], element: number) {
...
export async function run(root: string) {
    ...
    const generate = async (dir: Dirent[], actual: string, parent: string, parentReadMes: string[], imagenameStack: string[], parentDockers: string[]) => {
        let dockerfile = "";
        let readme = "";
        let variants: { key: string; values: string[], rename: string } = { key: '', values: [], rename: '' };
        let versions = false;
        const dirs: string[] = [];
        ...
        await Promise.all(dir.map(async (element) => {
            ...
            if (element.name === 'Dockerfile') {
                dockerfile = String(await fs.readFile(path));
            }

            if (element.name === 'README.md') {
                readme = String(await fs.readFile(path));
            }

            if (element.name === 'versions.json') {
                versions = true;
                variants = await JSON.parse(String(await fs.readFile(path)));
            }
        }));

        await Promise.all(dirs.map(async (elementName) => {
            const tempaltesDir = await fs.readdir(`${parent}/${actual}/${elementName}`, { withFileTypes: true });
            if (versions) {
                const { key, values, rename } = variants;

                return await doVariants(parent);

                async function doVariants(parent: string) {
    ...
    const tempaltesDir = await fs.readdir(root, { withFileTypes: true });
